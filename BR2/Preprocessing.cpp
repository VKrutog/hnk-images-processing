...
int Preprocessing::FewBlobsTop2(Mat* bmp)
{
	...
	for (int i = 0; i < lim; i++)
	{
		...
		{
			if (blobs[i].image.cols<= blobs[i].image.rows*1.5)
				few_blobs[counter++] = blobs[i];
		}
	}
...
bool Preprocessing::TicketYesNo_0_old(Mat* _src, int& counter)
{
	...
	SisThreshold(_src);
	//Erosion2(_src);
	//Preprocessing::RemoveBigSmallBlobs4(_src);
	//counter = FewBlobsTop2(_src);
	counter = Preprocessing::RemoveBigSmallBlobs6(_src);
	if (counter > 80)
	return true;
	else
	return false;
...
bool Preprocessing::RotationW3(Mat* _src, int& code, double& angle)
{

	double ret = 0.0;
	if (Preprocessing::BlackLines90(_src))
	{
		ret -= 90.0;
		Preprocessing::Rotate(_src, -90.);
	...
	Mat tmp;
	for (double i = -30.0; i < 30.0; i += 3.0)
	{
		tmp = _src[0].clone();
		Preprocessing::Rotate(&tmp, i);
		double res = Preprocessing::BlackLines(&tmp);
	...
	maxi = 0.0;
	double angle_ = 0.0;
	for (double i = angle - 3.0; i < angle + 3.0; i += 0.5)
	{
		tmp = _src[0].clone();
		Preprocessing::Rotate(&tmp, i);
		double res = Preprocessing::BlackLines(&tmp);
	...
	tmp = _src[0].clone();
	Preprocessing::Rotate(&tmp, angle_);
	double re = Preprocessing::GreenPercent(&tmp);
...
bool Preprocessing::JustCut(Mat* big, Mat* dop, Mat* dop2)
{
	...
	Preprocessing::ChangeFrame(big, beg, end);
	Preprocessing::ChangeFrame(dop, beg, end);
	Preprocessing::ChangeFrame(dop2, beg, end);
	return true;
}
...
bool Preprocessing::RotateAndCut(Mat* big, int& counter)
{
	...
	Preprocessing::TicketYesNo_0_old(big, counter1);
	Preprocessing::TicketYesNo_0(&tmp2, counter2);

	double r1 = IsItScan(big)/(counter1+1);
	double r2 = IsItScan(&tmp2)/(counter2+1);
	...
	Preprocessing::Rotate(&_src, ang);

	if (!res)
		return res;

	Point rez = Coord6(big);
	Point rez2 = Coord3(big);

	double length = rez.y - rez.x;

	int adds_left = 5. * length / 100.0;

	int adds_right = 5. * length / 100.0;



	if (rez.x - adds_left > 0)
		rez.x -= adds_left;

	if (rez.y + adds_right < big[0].cols - 1)
		rez.y += adds_right;



	Point beg(rez.x, rez2.x);
	Point end(rez.y, rez2.y);
	Preprocessing::ChangeFrame(&_src, beg, end);
...
Point Preprocessing::Coord(Mat* big)
{
	...
	int width = big[0].cols;
	int height = big[0].rows;
...
Point Preprocessing::Coord2(Mat* big)
{
	//imwrite("c:\\0000000aaa.jpg", big[0]);
	int stride_big = big[0].step;
	unsigned char *big_ = (unsigned char*)(big[0].data);
...
Point Preprocessing::Coord3(Mat* big)
{
	//imwrite("c:\\0000000aaa.jpg", big[0]);
	...
	for (int i = 0; i < white; i++)
	{
		...
		{
			int dopp_big = dop_big + j * mult;
			int dopp_big1 = dop_big + (j+1) * mult;
			int dopp_big2 = dop_big + (j+2) * mult;
			int dopp_big3 = dop_big + (j+3) * mult;
			int dopp_big4 = dop_big + (j + 4) * mult;
			int dopp_big5 = dop_big + (j + 5) * mult;
			int dopp_big6 = dop_big + (j + 6) * mult;


			bool cond = big_[dopp_big] == 0 && big_[dopp_big1] == 0 && big_[dopp_big2] == 0 && big_[dopp_big3] == 0 && big_[dopp_big4] == 0 && big_[dopp_big5] == 0 && big_[dopp_big6] == 0;
			bool cond_w = big_[dopp_big] == 255 || big_[dopp_big1] == 255 || big_[dopp_big2] == 255 || big_[dopp_big3] == 255 || big_[dopp_big4] == 255 || big_[dopp_big5] == 255 || big_[dopp_big6] == 255;
	...
	int bottom_black = -1;
	mini = 0;
	for (int i = big[0].rows-1; i >=white; i--)
	{
		int dop_big = stride_big * i;
		int cur = 0;
		int cur_w = 0;
	...
	Point ret(top_black, bottom_black);
	return ret;

}


Point Preprocessing::Coord4(Mat* big)
{
	//imwrite("c:\\0000000aaa.jpg", big[0]);
	int stride_big = big[0].step;
	unsigned char *big_ = (unsigned char*)(big[0].data);
	...
	{
		int cur = 0;
		for (int j = 0; j < big[0].rows; j++)
		{
			int dopp_big = stride_big * j + i * mult;
	...
	for (int i = 0; i < white; i++)
	{
		...
		{
			int dopp_big = stride_big * j + i * mult;
			int dopp_big1 = stride_big * j + (i+1) * mult;
			int dopp_big2 = stride_big * j + (i + 2) * mult;
			int dopp_big3 = stride_big * j + (i + 3) * mult;
			int dopp_big4 = stride_big * j + (i + 4) * mult;
			int dopp_big5 = stride_big * j + (i + 5) * mult;
			int dopp_big6 = stride_big * j + (i + 6) * mult;
	...
	int bottom_black = -1;
	mini = 0;
	for (int i = big[0].cols-1; i >= white; i--)
	{
		int cur = 0;
		for (int j = 0; j < big[0].rows; j++)
		{
...
Point Preprocessing::Coord5(Mat* big)
{
	//imwrite("c:\\0000000aaa.jpg", big[0]);
	...
	/*double cur_w_d = cur_w / big[0].rows;
	if (cur_w_d> lim_l)
		break;*/
	double lim_l = (1.*maxi / big[0].rows) / 6.;
	double lim_r = (1.*maxi / big[0].rows) / 6.;
	...
	for (int i = 0; i < white; i++)
	{
		...
		double cur_w_d = 1.*cur_w / big[0].rows;
		if (cur_w_d> lim_l)
			break;
	}
...
Point Preprocessing::Coord6(Mat* big)
{
	//imwrite("c:\\0000000aaa.jpg", big[0]);
	...
	double lim_l = 1.*maxi / 5.;
	double lim_r = 1.*maxi / 5.;
...
{
	if (bmp[0].cols > bmp[0].rows)
		return RemoveBigSmallBlobs3_h(bmp);
	else
		return RemoveBigSmallBlobs3_v(bmp);

}

int Preprocessing::RemoveBigSmallBlobs3_h(Mat* bmp)
{
	double from = 500;
	double till = 4000;
	int width = bmp[0].cols;
	...
	for (int i = 0; i < countB; i++)
	{
		...
		if (blobs[i].image.cols < bmp[0].cols / 10 && blobs[i].image.cols>10);
		{
			till += (blobs[i].image.cols);
			from += (blobs[i].image.cols);
			cc++;
		}
	}
	...
	for (int i = 0; i < countB; i++)
	{
		double area = blobs[i].area;
		bool tmp = true;// dif < 20;
		double prop = 1.* blobs[i].image.cols / blobs[i].image.rows;
		bool cond1 = !(prop > 2. /*&& blobs[i].image.rows > 20*/);
		//if (area > from && area <= till && tmp)
		if (blobs[i].image.cols > from && blobs[i].image.cols < till /*&& blobs[i].image.cols<bmp[0].cols / 2 */&& cond1)
		{
			few_blobs[counter++] = blobs[i];
		}
	}
...
int Preprocessing::RemoveBigSmallBlobs7(Mat* bmp)
{
	double from = 500;
	double till = 4000;
	int width = bmp[0].cols;
	...
	for (int i = 0; i < countB; i++)
	{
		...
		if (blobs[i].image.rows < bmp[0].rows / 10 && blobs[i].image.rows>10);
		{
	...
	for (int i = 0; i < countB; i++)
	{
		...
		bool cond1 = !(prop > 6 /*&& blobs[i].image.rows > 20*/);
		//if (area > from && area <= till && tmp)
	...
	Mat res_mat(bmp[0].rows, bmp[0].cols, CV_8UC1, Scalar(0));
	BlobsBuild_::PutBlobsBlackWhite(&res_mat, few_blobs, counter, 1);
	cvtColor(res_mat, bmp[0], COLOR_GRAY2BGR);
	res_mat.release();
	delete[] few_blobs;
	delete[] blobs;
...
int Preprocessing::RemoveBigSmallBlobs3_v(Mat* bmp)
{
	double from = 500;
	double till = 4000;
	int width = bmp[0].cols;
...
int Preprocessing::RemoveBigSmallBlobs6(Mat* bmp)
{
	double from = 500;
	double till = 4000;
	int width = bmp[0].cols;
	...
	for (int i = 0; i < countB; i++)
	{
		...
		bool cond1 = blobs[i].image.cols < bmp[0].cols / 10 && blobs[i].image.rows < bmp[0].rows / 10;
		bool cond2 = blobs[i].image.cols >10  && blobs[i].image.rows >10;
		bool cond3 = (blobs[i].image.cols < blobs[i].image.rows * 2) && (blobs[i].image.rows < blobs[i].image.cols * 2);
	...
	for (int i = 0; i < countB; i++)
	{
		...
		bool cond2 = blobs[i].image.cols >10 && blobs[i].image.rows >10;
		bool cond3 = (blobs[i].image.cols < blobs[i].image.rows * 2) && (blobs[i].image.rows < blobs[i].image.cols * 2);
		bool cond4 = blobs[i].image.cols > from && blobs[i].image.cols < till;
...
int Preprocessing::RemoveBigSmallBlobs5(Mat* bmp)
{
	double from = 500;
	double till = 4000;
	int width = bmp[0].cols;
...
bool Preprocessing::IsItBlob(Mat* big)
{
	int stride_big = big[0].step;
	unsigned char *big_ = (unsigned char*)(big[0].data);
	int val = 0;
	for (int i = 0; i < big[0].rows; i++)
	{
		...
		for (int j = 0; j < big[0].cols; j++)
		{
			int dopp_big = dop_big + j;

			
			if (big_[dopp_big] > 200)
				return true;
