...
std::string* Recognition::SetImage4(Mat* _src, int& string_counter)
{
	...
	////imwrite("c:\\0000000aaa1.jpg", src[0]);
	//Preprocessing::JustCut(src, &res_mat2, prcd);
	////imwrite("c:\\0000000aaa2.jpg", src[0]);
...
bool Recognition::PrePro2(Mat* src, Mat* big, int& counter)
{
	...
	//imwrite("c:\\0000000aaa4.jpg", big[0]);
	return res;
}
...
Mat Recognition::ReProcessing3(Mat* _src)
{
	...
	//for (int i = 0; i < blobs_counter; i++)
	//{
	//	ar += (blobs[i].image.rows*blobs[i].image.cols);
	//}

	//ar /= blobs_counter;
	//double sm_ar = ar - 80 * ar / 100;
	//double bg_ar = ar + 100 * ar / 100;
	...
	//{
	//	
	//	double tmp= (blobs[i].image.rows*blobs[i].image.cols);
	//	bool qw = Preprocessing::IsItBlob(&blobs[i].image);
	//	if (tmp > sm_ar && tmp < bg_ar && qw)
	//		new_counter++;
	//}
	...
	////Blob_* blobs = new Blob_[blobs_counter];

	//for (int i = 0; i < blobs_counter; i++)
	//{
	//	/*if (&blobs_new[i].image == NULL)
	//	{
	//	int yu = 789;
	//	}*/
